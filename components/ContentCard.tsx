...
export const ContentCard: React.FC<ContentCardProps> = ({ technique }) => {
    ...
    return (
        <article className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-lg dark:shadow-2xl dark:shadow-slate-900/50 border border-stone-200/80 dark:border-slate-700/80 transition-shadow hover:shadow-xl">
            <h2 className="text-3xl font-bold text-stone-800 dark:text-white">{technique.title}</h2>
            
            <div className="mt-6 space-y-8">
                <div>
                    <h3 className="font-semibold text-lg text-teal-700 dark:text-teal-400 mb-2">¿Qué es?</h3>
                    <p className="text-stone-600 dark:text-stone-300 leading-relaxed">{technique.explanation}</p>
                </div>
                
                <div>
                    <h3 className="font-semibold text-lg text-teal-700 dark:text-teal-400 mb-2">Analogía Simple</h3>
                    <div className="flex items-start gap-3 bg-stone-50 dark:bg-slate-700/50 p-4 rounded-lg border border-stone-200 dark:border-slate-600">
                        ...
                        <p className="text-stone-600 dark:text-stone-300 italic">"{technique.analogy}"</p>
                    </div>
                </div>

                <div>
                    <h3 className="font-semibold text-lg text-teal-700 dark:text-teal-400 mb-2">Diagrama de Flujo</h3>
                ...
                <div>
                    <h3 className="font-semibold text-lg text-teal-700 dark:text-teal-400 mb-2">Ejemplo y Plantilla</h3>
                    <div className="bg-slate-800 dark:bg-slate-900 rounded-lg overflow-hidden border border-stone-200/10">
                        ...
                        <div className="px-4 py-2 bg-stone-700 dark:bg-slate-700 text-stone-300 text-sm font-mono flex justify-between items-center">
                            ...
                            >
                                {isCopied ? <><CheckIcon /> Copiado</> : <><CopyIcon /> Copiar</>}
                            </button>
                        </div>
